package deprectaed.store.io.readers;
...
import deprecated.store.io.RecordMapper;
...
public class Parametric_FormatReader implements FormatReader {
	...
	public Vector<String[]> readRecords(int month) throws NotValidMonthException {
		if (isValidMonth(month)) {
			...
			throw new NotValidMonthException(
					"The month [" + month + "] is not among the valid ones [" + validMonths + "]");
		}
	}
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if (i >= recordMapper.getFirstValidRecord()) {
				...
				if (tfe.isValid(record)) {
					...
					Date date = new SimpleDateFormat(recordMapper.getDateFormat())
							.parse(record[recordMapper.getDateField()]);
					// long amount =
					// MoneyConverter.parseDecimalStringToLong(record[recordMapper.getAmountField()]);
					long amount = MoneyConverter.parseDecimalStringToLong(record[recordMapper.getAmountField()]);
					// String recString = (new DecimalFormat("##.00").format(((double) amount) /
					// 100));

					normalizedRecord[0] = (new SimpleDateFormat(Transaction.DATE_FORMAT)).format(date); // date

					normalizedRecord[1] = amount + ""; // amount
					normalizedRecord[2] = ""; // amount decimal
					...
					// System.out.println("ADD ["+month+"] FOR "+date.toString()+ " BY
					// "+record[recordMapper.getDateField()]);

					// if(!transactionsMap.containsKey(month)){transactionsMap.put(month, new
					// Vector<>());}
	...
	public void loadInputStream(InputStream is, String fileName) throws Exception {
		String[] items = fileName.split(NORMALIZED_SEPARATOR_REGEXP);
		long endBalance = MoneyConverter.parseDecimalStringToLong(items[items.length - 3], items[items.length - 2]);
		Vector<String[]> records = IO.readItems(is, recordMapper.getFields());
