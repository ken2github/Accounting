...
import deprecated.indexes.nets.YEAR_MONTH_CATEGORY_SUBCATEGORY_COMMON;
import deprecated.model.books.YearBook;
import deprecated.model.schema.Transaction;

public class AVERAGE_YEAR_CATEGORY_SUBCATEGORY_COMMON extends MapIndex {
	...
	private static Class<? extends MapIndex> getMyClass(){
		return AVERAGE_YEAR_CATEGORY_SUBCATEGORY_COMMON.class;
	}
	...
	private static Map<String,Long> computeYearIndex(YearBook yb) {
		...
		//List<String> flows = Arrays.asList(new String[]{"INPUT","OUTPUT"});
				for (String s : sectors) {
					for (String cmm : commons) {
						//for (String f : flows) {
							...
							String id = Utils.delimiterizing(Constants.AVERAGE,year,category,subcategory,common);
				...
				Map<String,Long> depMap = YEAR_MONTH_CATEGORY_SUBCATEGORY_COMMON.getIndexMap(yb);
