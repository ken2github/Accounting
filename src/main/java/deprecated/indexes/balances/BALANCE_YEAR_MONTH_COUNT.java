package deprecated.indexes.balances;
...
public class BALANCE_YEAR_MONTH_COUNT extends MapIndex {
	...
	private static Class<? extends MapIndex> getMyClass(){
		return BALANCE_YEAR_MONTH_COUNT.class;
	}
	...
	private static Map<String,Long> computeYearIndex(YearBook yb) {
		...
		for (MonthBook mb : yb.getMonthBooks()) {
			...
			for (CountMonthTransactions cmt : mb.getListOfcountMonthTransactions()) {
				String id = Utils.delimiterizing(Constants.BALANCE,year,month,cmt.getCount());
								
				// Int with balance of month
				if(!map.containsKey(id)) {
					map.put(id, cmt.getFinalBalance());
				}
			}
				...
				for (String mm : months) {
					//for (String cmm : commons) {
						for (String c : counts) {
							...
							String id = Utils.delimiterizing(Constants.BALANCE,year,month,count);
