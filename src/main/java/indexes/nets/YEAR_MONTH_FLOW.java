...
public class YEAR_MONTH_FLOW extends MapIndex {
	...
	private static Class<? extends MapIndex> getMyClass(){
		return YEAR_MONTH_FLOW.class;
	}

	private static Map<String,Double> computeYearIndex(YearBook yb) {
		Map<String,Double> map = new HashMap<>();
		
		Map<String,Double> depMap = YEAR_MONTH_COMMON_FLOW.getIndexMap(yb);
		...
		for (String key : depMap.keySet()) {
			...
			String id = Utils.delimiterizing(year,month,flow);
