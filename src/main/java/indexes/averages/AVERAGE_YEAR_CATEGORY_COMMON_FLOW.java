...
public class AVERAGE_YEAR_CATEGORY_COMMON_FLOW extends MapIndex {
	...
	private static Class<? extends MapIndex> getMyClass(){
		return AVERAGE_YEAR_CATEGORY_COMMON_FLOW.class;
	}
	...
	private static Map<String,Double> computeYearIndex(YearBook yb) {
		...
		List<String> flows = Arrays.asList(new String[]{"INPUT","OUTPUT"});
				for (String ss : supersectors) {
					for (String cmm : commons) {
						for (String f : flows) {
							...
							String id = Utils.delimiterizing(Constants.AVERAGE,year,category,common,flow);
				...
				Map<String,Double> depMap = YEAR_MONTH_CATEGORY_COMMON_FLOW.getIndexMap(yb);
