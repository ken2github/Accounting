...
public class AVERAGE_YEAR_CATEGORY_SUBCATEGORY extends MapIndex {
	...
	private static Class<? extends MapIndex> getMyClass(){
		return AVERAGE_YEAR_CATEGORY_SUBCATEGORY.class;
	}
	...
	private static Map<String,Double> computeYearIndex(YearBook yb) {
		...
		//List<String> flows = Arrays.asList(new String[]{"INPUT","OUTPUT"});
				for (String s : sectors) {
					//for (String cmm : commons) {
						//for (String f : flows) {
							...
							String id = Utils.delimiterizing(Constants.AVERAGE,year,category,subcategory);
				...
				Map<String,Double> depMap = YEAR_MONTH_CATEGORY_SUBCATEGORY.getIndexMap(yb);
